...
def list_amazon_linux_2_amis(ec2):
    # Lister les AMIs Amazon Linux 2 disponibles
    ...
    if amis:
        print("AMIs Amazon Linux 2 disponibles :")
        for ami in amis:
            print(f"ID de l'AMI: {ami['ImageId']} - Nom: {ami['Name']}")
    else:
        print("Aucune AMI Amazon Linux 2 disponible trouvée.")
...
def check_existing_resources(ec2, key_name, vpc_name, subnet_name):
    try:
        # Vérification si la clé existe déjà
        ec2.describe_key_pairs(KeyNames=[key_name])
        print(f"La clé {key_name} existe déjà.")
    except ClientError:
        print(f"La clé {key_name} n'existe pas.")
    
    # Vérification si le VPC existe déjà
    vpc_response = ec2.describe_vpcs(Filters=[{'Name': 'tag:Name', 'Values': [vpc_name]}])
    if vpc_response['Vpcs']:
        print(f"VPC '{vpc_name}' existe déjà avec ID : {vpc_response['Vpcs'][0]['VpcId']}")
    else:
        print(f"Le VPC '{vpc_name}' n'existe pas.")

    # Vérification si le sous-réseau existe déjà
    subnet_response = ec2.describe_subnets(Filters=[{'Name': 'tag:Name', 'Values': [subnet_name]}])
    if subnet_response['Subnets']:
        print(f"Sous-réseau '{subnet_name}' existe déjà avec ID : {subnet_response['Subnets'][0]['SubnetId']}")
    else:
        print(f"Le sous-réseau '{subnet_name}' n'existe pas.")
...
# Liste des AMIs Amazon Linux 2 disponibles
list_amazon_linux_2_amis(ec2)

# Vérification des ressources existantes
check_existing_resources(ec2, key_name, vpc_name, public_subnet_name)
...
try:
    ...
    ec2.create_tags(Resources=[public_subnet_id], Tags=[{'Key': 'Name', 'Value': public_subnet_name}])
    ...
    ec2.create_tags(Resources=[private_subnet_id], Tags=[{'Key': 'Name', 'Value': private_subnet_name}])
